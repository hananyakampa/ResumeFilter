...
UPLOAD_PATH=r'saved_files\resume_files'
UPLOAD_PATH1=r'saved_files\job_description_file'
PDF_PATH=r'saved_files\pdf_files'
CSV_PATH=r'saved_files\csv_file'
...
def success():  
    ...
    numbe=[]
    for x1 in res_files:
        pn = re.findall(r'(?:\+?\d{2}[ -]?)?\d{10}', x1)
        numbe.append(pn)

    
    data = pd.DataFrame({'Resume File Name' : candidate_resume, 'Matching %' : res,'Email List':email,'Name List':names_,'Mobile_Number':numbe}).sort_values(['Matching %'],ascending=False).reset_index(drop=True)
    ...
    # reading the csv file
        data = pd.read_csv(r"Skills_File/jd_skills.csv") 
